...
JWT化の第一段階として、ログイン成功時に `accessToken`、`tokenType`、`expiresIn` を返す実装まで追加済みです。

ただし、現時点では `/api/me/**` の認証はまだセッションCookie方式で動いています。
`Authorization: Bearer {token}` からログインユーザーを復元する設定は、次の実装対象です。
...
将来的には、`GET /api/auth/me` もJWTの `Authorization` ヘッダーからログインユーザーを取得します。
現時点では、まだセッションCookie方式でも動作します。
