...
JWT実装では、第一候補として Spring Security の Resource Server / JOSE 系を使います。
...
- 実装前に、現在のSpring Boot / Spring Securityのバージョンで必要な依存と設定を確認する。

## API仕様変更案

JWT化後のログインレスポンスは、現在の `user` に加えて `accessToken` と `expiresIn` を返します。
...
`GET /api/auth/me` は、JWTの `Authorization` ヘッダーからログインユーザーを取得します。
...
1. Spring Security OAuth2 Resource Server / JOSE系の依存を追加する。
2. JWT署名鍵と有効期限を `application.yml` または環境変数で設定できるようにする。
3. JWT生成・検証用の設定またはutilityを追加する。
4. `LoginResponse` に `accessToken`、`tokenType`、`expiresIn` を追加する。
5. ログイン成功時にaccess tokenを返す。
6. FEでtokenを `sessionStorage` に保持する。
7. `fetchClient.ts` で `Authorization` ヘッダーを付けられるようにする。
8. BEでJWTから認証情報を復元する。
9. `/api/me/**` をJWT認証で保護する。
10. 未ログイン、期限切れ、改ざんtokenのエラーレスポンスを確認する。
11. BE / FEのテストを追加する。
